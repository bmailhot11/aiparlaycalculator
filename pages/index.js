...
function ParlayResultsModal({ parlay, isOpen, onClose, handleUpgrade }) {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      const canvas = await html2canvas(parlayCardRef.current, {
        ...
        scrollX: 0,
        scrollY: 0,
        windowWidth: parlayCardRef.current.scrollWidth,
        windowHeight: parlayCardRef.current.scrollHeight
      })
  ...
  const handleShare = async () => {
    const parlayDetails = parlay.parlay_legs?.map(leg => 
      `• ${leg.game || 'Game'}: ${leg.selection || 'Bet'} (${leg.odds || 'N/A'})`
    ).join('\n') || 'Professional Analysis'
    
    const shareText = `🎯 ${parlay.parlay_legs?.length || 0}-Leg ${parlay.sport || 'Multi-Sport'} Parlay Analysis\n\n${parlayDetails}\n\nTotal Odds: ${parlay.total_american_odds || parlay.total_odds || 'TBD'}\nConfidence: ${parlay.confidence || 'Medium confidence'}\nRisk Level: ${parlay.risk_assessment || 'Calculated'}\n\n🤖 Generated by AiParlayCalculator`
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-gray-800 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-700">
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div 
            ref={parlayCardRef} 
            className="bg-gray-900 rounded-xl p-4 sm:p-6 border border-gray-700"
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between gap-3 mb-4 sm:mb-6">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-green-400 font-bold text-sm sm:text-lg">AiParlayCalculator</span>
                  <span className="bg-green-500/20 text-green-400 px-2 py-0.5 sm:py-1 rounded text-xs sm:text-sm font-medium">
                    ANALYSIS
                  </span>
                </div>
            ...
            <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
              {(parlay.parlay_legs || []).map((leg, index) => (
                <div key={index} className="bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-600">
