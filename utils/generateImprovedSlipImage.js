...
export async function generateImprovedSlipImage({ originalSlip, improvedBets, explanation, analysis }) {
  ...
  // Try to use server-side canvas first, fall back to browser canvas
  let canvas, ctx;
  
  try {
    if (typeof window === 'undefined' || typeof document === 'undefined') {
      console.log('🖥️ Using server-side canvas rendering...');
      
      // Try to import canvas library for server-side rendering
      try {
        const { createCanvas } = await import('canvas');
        canvas = createCanvas(800, 1000);
        ctx = canvas.getContext('2d');
        console.log('✅ Server-side canvas initialized');
      } catch (canvasError) {
        console.log('⚠️ Server-side canvas not available, generating fallback response');
        
        // Return a fallback response with structured data instead of null
        return {
          type: 'fallback',
          slip_data: {
            bet_id: generateBetId(),
            stakes: originalSlip?.total_stake || '$100',
            legs: improvedBets || [],
            total_odds: calculateTotalOdds(improvedBets || []),
            potential_payout: calculateImprovedPayout(improvedBets || [], originalSlip?.total_stake || '$100'),
            explanation: explanation || 'Your bet slip has been optimized for better odds.'
          },
          message: 'Slip data prepared - visual template generation requires canvas setup'
        };
      }
    } else {
      console.log('🌐 Using browser canvas...');
  ...
  // Set canvas dimensions (works for both browser and server-side canvas)
  ...
  // Handle both browser and server-side canvas
  let dataUrl;
  if (canvas.toDataURL) {
    // Browser canvas
    dataUrl = canvas.toDataURL('image/png', 0.95);
  } else if (canvas.toBuffer) {
    // Server-side canvas (node-canvas)
    const buffer = canvas.toBuffer('image/png');
    dataUrl = `data:image/png;base64,${buffer.toString('base64')}`;
  } else {
    console.error('❌ Canvas does not support data URL generation');
    return null;
  }
