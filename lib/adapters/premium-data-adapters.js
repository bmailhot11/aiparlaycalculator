...
export const fetchLivePreviewData = async () => {
  ...
  // Fetch real AI parlay suggestions
  try {
    console.log('Fetching real AI parlay suggestions...');
    const parlayController = new AbortController();
    setTimeout(() => parlayController.abort(), 3000);
    ...
    if (parlayResponse.ok) {
      ...
      if (parlayData.success && parlayData.parlay) {
        ...
        console.log(`✅ Found real AI parlay: ${parlayData.parlay.legs.length} legs`);
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (perfResponse.ok) {
      ...
      if (perfData.success && perfData.data.performanceData) {
        results.clv = perfData.data.performanceData;
        console.log(`✅ Found real performance data: ${results.clv.length} data points`);
      }
    }
  } catch (error) {
