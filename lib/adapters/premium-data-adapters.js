...
export const fetchLivePreviewData = async () => {
  ...
  // Skip other API calls for now to avoid further delays
  console.log('Skipping other API calls to avoid delays');
  ...
  // Return transformed data - only real data, empty arrays/null for missing data
  return {
    evFeed: toPreviewEV(results.evFeed),
    arbitrage: toArbRows(results.arbitrage), 
    lineShop: toLineShop(results.lineShop),
