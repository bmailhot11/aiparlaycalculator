...
// Try to load Supabase, but don't fail if it's not available
let supabase;
try {
  const client = require('../utils/supabaseClient');
  supabase = client.supabase;
} catch (error) {
  console.log('Supabase not available for historical analysis');
  supabase = null;
}
...
async function analyzeUnderdogPatterns(sport, daysBack = 365) {
  if (!supabase) {
    console.log('Supabase not available for underdog analysis');
    return [];
  }
