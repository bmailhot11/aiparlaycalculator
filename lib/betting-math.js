 ...
 */
async function processLeg(bet, bookmakers, historicalData = null) {
  // Extract odds
  ...
  let priorProb = historicalData?.hit_rate || null;
  
  if (!priorProb && bet.sport && bet.market_type) {
    try {
      const historicalDataService = require('./historical-data-service');
      
      // Extract team from selection or game info
      let team = null;
      if (bet.game) {
        const teams = bet.game.split(/[@vs]/i).map(t => t.trim());
        // Try to match selection to a team
        team = teams.find(t => bet.selection.includes(t)) || teams[0];
      }
      ...
      if (historicalStats && historicalStats.confidence !== 'very_low') {
        priorProb = historicalStats.hit_rate;
        
        // Adjust weight based on confidence
        if (historicalStats.confidence === 'low') {
          priorProb = priorProb * 0.7 + 0.5 * 0.3; // Blend with 50% baseline
        }
      }
    } catch (error) {
 ...
 */
async function processParlay(legs, bookmakers) {
  // Process each leg (now async)
  const processedLegs = await Promise.all(
    legs.map(leg => processLeg(leg, bookmakers))
  );
 ...
 */
async function analyzeSlip(legs, bookmakers) {
  const analysis = await processParlay(legs, bookmakers);
