...
class OptimizedEVFetcher {
  ...
  async getPreFilteredCandidates(sport, preferences) {
    ...
    // Check cache first - include sport in cache key for proper isolation
    const cacheKey = `prefilter_${sport}_${JSON.stringify(preferences)}`;
    const cached = this.getCachedData(cacheKey, this.CACHE_TTL);
    if (cached) {
      console.log(`⚡ [OptimizedEV] Using cached pre-filter results for ${sport}`);
      return cached;
    }
  ...
  async getCachedEventsOdds(sport) {
    try {
      ...
      // Clear stale cache if sport changes (to prevent cross-sport contamination)
      const lastSportKey = `last_sport_requested`;
      const lastSport = this.apiCache.get(lastSportKey);
      if (lastSport && lastSport !== sport) {
        console.log(`🔄 [OptimizedEV] Sport changed from ${lastSport} to ${sport}, clearing relevant caches`);
        // Clear sport-specific caches
        for (const [key] of this.apiCache) {
          if (key.includes(lastSport) || key.includes('prefilter')) {
            this.apiCache.delete(key);
          }
        }
      }
      this.apiCache.set(lastSportKey, sport);
      ...
      // Validate that returned odds are for the correct sport
      const validOddsData = oddsData.filter(game => {
        // Check if this game belongs to events from the requested sport
      ...
      console.log(`✅ [OptimizedEV] Cached approach: ${upcomingEvents.length} events → ${validOddsData.length} games with odds for ${sport}`);
