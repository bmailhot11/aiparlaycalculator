...
class OptimizedEVFetcher {
  ...
  async fetchOptimalEVLines(sport, preferences = {}) {
    ...
    try {
      ...
      const positiveEVLines = evLines.filter(line => line.expected_value > 0.005); // 0.5% minimum for final display
  ...
  async getMinimalOddsData(sport) {
    ...
    for (const sportKey of sportKeys) {
      try {
        ...
        // Use different regions for different sports
        const regions = sportKey.startsWith('soccer_') ? 'us,uk,eu' : 'us';
        const apiUrl = `https://api.the-odds-api.com/v4/sports/${sportKey}/odds/?apiKey=${API_KEY}&regions=${regions}&markets=h2h&oddsFormat=american&dateFormat=iso`;
