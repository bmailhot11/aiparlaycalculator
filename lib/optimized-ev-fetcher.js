...
// Optimized EV+ line fetching system with Supabase caching
const eventsCache = require('./events-cache.js');
const supabaseCache = require('./supabase-cache.js');

class OptimizedEVFetcher {
  constructor() {
    this.probabilityModels = new Map();
    this.MIN_EV_THRESHOLD = -0.15; // Very lenient threshold to find more opportunities
    this.MAX_API_CALLS_PER_REQUEST = 2; // Further reduced API calls with Supabase
    this.supabaseCache = supabaseCache;
  }
  ...
  async fetchOptimalEVLines(sport, preferences = {}) {
    ...
    try {
      ...
      const positiveEVLines = evLines.filter(line => line.expected_value > -0.01); // -1% minimum for final display (more lenient)
      
      // Add sport tag to each line for verification
      const taggedLines = positiveEVLines.map(line => ({
        ...line,
      ...
      console.log(`✅ [OptimizedEV] Completed in ${duration}ms, found ${taggedLines.length} positive EV lines for ${sport}, ${arbitrageOpportunities.length} arbitrage opportunities`);
      ...
      return {
        ...
        performance: {
          ...
          positive_ev_found: taggedLines.length,
  ...
  // OPTIMIZATION 1: Pre-filter using mathematical shortcuts with fallback logic
  async getPreFilteredCandidates(sport, preferences) {
    console.log(`🎯 [OptimizedEV] Pre-filtering candidates for ${sport}`);
    
    // Check Supabase cache first for EV data
    const cached = await this.supabaseCache.getEVData(sport);
    if (cached) {
      console.log(`⚡ [OptimizedEV] Using Supabase cached EV results for ${sport}`);
      return cached;
    }

    // NEW: Use Supabase cached odds for better performance
    const oddsData = await this.getSupabaseCachedOdds(sport);
    if (!oddsData || oddsData.length === 0) {
      console.log(`⚠️ [OptimizedEV] No odds data available for ${sport}`);
      return [];
    }

    // First pass: try to find candidates with all market types (props + main markets)
    let candidates = this.filterCandidatesWithMarkets(oddsData, sport, ['h2h', 'spreads', 'totals'], true);
    
    // Fallback: if player props don't work, use all main markets (h2h, spreads, totals) without props
    if (candidates.length === 0) {
      console.log(`🔄 [OptimizedEV] No candidates found with player props, falling back to all main markets without props`);
      candidates = this.filterCandidatesWithMarkets(oddsData, sport, ['h2h', 'spreads', 'totals'], false);
    }
    
    // Second fallback: if still no candidates, try just spreads and totals (often have value)
    if (candidates.length === 0) {
      console.log(`🔄 [OptimizedEV] Still no candidates, focusing on spreads and totals markets`);
      candidates = this.filterCandidatesWithMarkets(oddsData, sport, ['spreads', 'totals'], false);
    }
    
    // Final fallback: h2h only if nothing else works
    if (candidates.length === 0) {
      console.log(`🔄 [OptimizedEV] Final fallback to h2h markets only`);
      candidates = this.filterCandidatesWithMarkets(oddsData, sport, ['h2h'], false);
    }

    // Cache the filtered candidates in Supabase
    await this.supabaseCache.cacheEVData(sport, candidates);
  ...
  // Helper method to filter candidates with specific market types
  filterCandidatesWithMarkets(oddsData, sport, allowedMainMarkets, includePlayerProps = true) {
    const candidates = [];
    ...
    for (const game of oddsData) {
      ...
      for (const bookmaker of game.bookmakers) { // Use all bookmakers for maximum arbitrage opportunities
        ...
        for (const market of bookmaker.markets) {
          // Include specified main markets AND optionally player props
          const isMainMarket = allowedMainMarkets.includes(market.key);
          const isPlayerProp = includePlayerProps && market.key.startsWith('player_');
          
          if (!isMainMarket && !isPlayerProp) continue;
          
          for (const outcome of market.outcomes || []) {
            // Validate outcome data (especially important for player props)
            if (!outcome.name || !outcome.price || isNaN(outcome.price)) {
              console.log(`⚠️ [OptimizedEV] Skipping invalid outcome:`, outcome);
              continue;
            }
            ...
            if (quickEV > this.MIN_EV_THRESHOLD) {
              // Create enhanced selection description with point/line information
              let enhancedSelection = outcome.name;
              
              // For player props, enhance with point information
              if (market.key.startsWith('player_') && outcome.point !== undefined) {
                ...
                // For spreads/totals with points
  ...
  // Removed: Replaced with getSupabaseCachedOdds method
  ...
  getVigEstimate(marketKey) {
    const vigEstimates = {
      ...
      'player_points': 0.04,     // 4% for player points
      'player_assists': 0.045,   // 4.5% for assists
      'player_rebounds': 0.04,   // 4% for rebounds
      'player_pass_yds': 0.042,  // 4.2% for passing yards
      'player_rush_yds': 0.038,  // 3.8% for rushing yards
      'player_reception_yds': 0.04, // 4% for receiving yards
      'player_pass_tds': 0.045,  // 4.5% for pass TDs
      'player_rush_tds': 0.043,  // 4.3% for rush TDs
      'player_threes': 0.041,    // 4.1% for made 3s
      'player_double_double': 0.05, // 5% for double-double
      'player_goals': 0.046,     // 4.6% for goals
      'player_shots_on_goal': 0.037 // 3.7% for shots on goal
    };
    
    // For any player prop not specifically listed, use higher vig
    if (marketKey.startsWith('player_')) {
      return vigEstimates[marketKey] || 0.045; // Default 4.5% for player props
    }
    
    return vigEstimates[marketKey] || 0.03; // Default 3% for other markets
  }
  ...
  applyMarketAdjustments(prob, outcome, americanOdds, game = null) {
    ...
    // Player prop patterns - more sophisticated adjustments
    if (outcome.name.toLowerCase().includes('over')) {
      // Overs often juiced by public betting, especially for popular players
      adjusted *= americanOdds > -120 ? 0.96 : 0.94; // Less adjustment for closer to even odds
    } else if (outcome.name.toLowerCase().includes('under')) {
      // Unders sometimes have value, especially in higher scoring games
      adjusted *= americanOdds > -120 ? 1.03 : 1.05; // More conservative adjustment for favored unders
    }
    
    // Additional player prop adjustments based on selection type
    const selectionLower = outcome.name.toLowerCase();
    if (selectionLower.includes('anytime') || selectionLower.includes('touchdown')) {
      // Anytime TD and scoring props often have public bias
      adjusted *= 0.97;
    } else if (selectionLower.includes('yards') && americanOdds > 150) {
      // Long shot yardage props can have value
      adjusted *= 1.02;
    }
  ...
  async calculateBatchEVWithEarlyTermination(candidates) {
    ...
    for (let i = 0; i < candidates.length; i += batchSize) {
      ...
      for (const candidate of batch) {
        // Calculate precise EV (no individual caching for simplicity)
        const preciseEV = await this.calculatePreciseEV(candidate);
        const cachedEV = preciseEV;
  ...
  async getMinimalOddsData(sport) {
    ...
    // No cache for fallback method - use direct API
    ...
    for (const sportKey of sportKeys) {
      try {
        ...
        // Use AbortController for proper timeout in Node.js
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 8000);
        
        const response = await fetch(apiUrl, { 
          signal: controller.signal,
          headers: { 'User-Agent': 'AIParlay/1.0' }
        });
        ...
        if (error.name === 'AbortError') {
          console.log(`⏰ [OptimizedEV] Timeout for ${sportKey}, trying next variant...`);
        } else {
          console.log(`❌ [OptimizedEV] Error with ${sportKey}:`, error.message);
        }
        continue;
      }
    }
    ...
    // Smart date filtering: include live games and upcoming games
    const now = new Date();
    const threeHoursAgo = new Date(now.getTime() - 3 * 60 * 60 * 1000); // Include games that started up to 3 hours ago
    const sevenDaysFromNow = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
    const thirtyDaysFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);

    // First, try games that are live or in next 7 days
    const gamesNext7Days = validGames.filter(game => {
      if (!game.commence_time) return true; // Include games without start time
      const gameTime = new Date(game.commence_time);
      return gameTime >= threeHoursAgo && gameTime <= sevenDaysFromNow; // Include live games
    });
    ...
    if (gamesNext7Days.length < 7) {
      const gamesNext30Days = validGames.filter(game => {
        if (!game.commence_time) return true; // Include games without start time
        const gameTime = new Date(game.commence_time);
        return gameTime >= threeHoursAgo && gameTime <= thirtyDaysFromNow; // Include live games
      });
    ...
    // Data fetched directly, no caching for fallback method
    
    console.log(`📊 [OptimizedEV] Fetched minimal data: ${filteredGames.length} games, all available sportsbooks`);
    return filteredGames;
  }

  // Supabase-based odds fetching with 5-minute cache
  async getSupabaseCachedOdds(sport) {
    try {
      console.log(`🎯 [OptimizedEV] Checking Supabase cache for ${sport} odds`);
      
      // Check Supabase cache first (5-minute TTL)
      const cached = await this.supabaseCache.getOdds(sport);
      if (cached && cached.length > 0) {
        console.log(`⚡ [OptimizedEV] Using Supabase cached odds for ${sport} - ${cached.length} games`);
        return cached;
      }
      
      console.log(`🌐 [OptimizedEV] Cache miss, fetching fresh odds for ${sport}`);
      
      // Fallback to cached events approach for fresh data
      ...
      const oddsData = await eventsCache.getOddsForEvents(upcomingEvents, markets, true);
      
      // Validate and cache the fresh data
      const validOddsData = oddsData.filter(game => {
      ...
      // Cache in Supabase for 5 minutes
      if (validOddsData.length > 0) {
        await this.supabaseCache.cacheOdds(sport, validOddsData);
        console.log(`✅ [OptimizedEV] Cached ${validOddsData.length} games for ${sport} in Supabase`);
      }
      
      return validOddsData;
      
    } catch (error) {
      console.log(`❌ [OptimizedEV] Supabase cache failed, using fallback:`, error.message);
      
      // Return empty array if all systems fail to prevent infinite loops
      try {
        return await this.getMinimalOddsData(sport);
      } catch (fallbackError) {
        console.log(`❌ [OptimizedEV] All data sources failed:`, fallbackError.message);
        return [];
      }
    }
  }

  // Cache stats now handled by Supabase cache
  async getCacheStats() {
    return await this.supabaseCache.getCacheStats();
  }
...
module.exports = optimizedEVFetcher;
module.exports.default = optimizedEVFetcher;